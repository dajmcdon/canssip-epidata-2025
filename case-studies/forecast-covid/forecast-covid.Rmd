...
Can we perform split-sample using `{epipredict}`?
...
arx_forecaster(epi_data = train %>% as_epi_df(), 
               ...
               args_list = arx_args_list(lags = k-1, ahead = 1L))$predictions
```
...
- Fit AR using epi-slide, check if error goes down

- Fit ARX using epi-slide and including lagged cases and deaths as predictors,
check if error goes down

- Is there an automatic way to select best lags within epipredict 
(e.g. using Lasso somehow)?

- Introduce probabilistic forecasters (quantile regression) and appropriate 
evaluation methods
