...
* At test time $t,$ take provisional value to be $Y_s,$ where $s$ is the largest occurence date among all values reported up until time $t.$
...
* Let's start "simple": let's try predicting $Y_t$ with whichever of $Y_{t - 1}$
  and $Y_{t - 2}$ are available in version $t$.
