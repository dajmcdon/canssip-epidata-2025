...
ggplot() +
  geom_line(data = mortality_latest, aes(x = time_value, y = mortality)) +
...
```{r finalized-data-as-of-March-10}
#| echo: true
...
```{r boxplot-ratio}
#| echo: false
ratio_df <- data.frame(ratio_real_time_to_mar10, mean_ratio = mean(ratio_real_time_to_mar10))

# Create the boxplot with mean marked as a bold cross
ggplot(ratio_df, aes(y = ratio_real_time_to_mar10)) +
  ...
  coord_cartesian(ylim = c(0, 0.5))  # Limit y-axis between 0 and 0.5 to zoom in
```
<div style="font-size: 0.8em;">
