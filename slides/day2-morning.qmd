...
## Fit ARX with 2 and 3 lags on trailing window
...
for (t in t0:(n-h)) {
  ...
  yhat_ridge[t+h] = yhat_ridge_mat[t+h, ridge_index]
  yhat_lasso[t+h] = yhat_lasso_mat[t+h, lasso_index]
  # Prediction error
  mae_ridge = colMeans(abs(yhat_ridge_mat - y), na.rm = T)
  mae_lasso = colMeans(abs(yhat_lasso_mat - y), na.rm = T)
