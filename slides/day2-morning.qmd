...
#| fig-width: 7
rq_pred_trailing <- cbind(ca, pred_interval_rq)
...
```{r qr-errors-arx}
getErrors(y_test, pred_interval_rq[!is.na(pred_interval_rq)], "rq.trailing")
#mean(pred_interval_rq[,1] <= pred_interval_rq[,2] & 
#       pred_interval_rq[,2] <= pred_interval_rq[,3], na.rm = T)
```
...
```{r q-reg-more-quantiles}
# Initialize matrix to store predictions 
# 3 columns: lower limit, median, and upper limit
pred_interval_rq_23 <- matrix(NA, nrow = n, ncol = 23)

for (t in t0:(n-h)) {
  # Fit quantile regression
  rq_trailing = rq(deaths ~ lagged_deaths + lagged_cases, 
                   tau = c(0.01, 0.025, 1:19 / 20, 0.975, 0.99),
                   data = ca, subset = (1:n) <= t & (1:n) > (t-w)) 

  pred_interval_rq_23[t+h, ] = pmax(0, predict(rq_trailing, newdata = data.frame(ca[t+h, ])))
}
```

```{r q-reg-plot-more-quantiles}
#| fig-width: 7
cbind(ca, pred_interval_rq_23) |>
  mutate(observed = deaths, 
         `predicted (not sorted)` = `...18`) |>
  pivot_longer(cols = c(`predicted (not sorted)`, observed), 
               names_to = 'Deaths') |>
  ggplot(aes(time_value, value)) +
  geom_ribbon(aes(ymin = `...7`, ymax = `...29`), alpha = .1, fill = primary) +
  geom_ribbon(aes(ymin = `...8`, ymax = `...28`), alpha = .1, fill = primary) +
  geom_ribbon(aes(ymin = `...9`, ymax = `...27`), alpha = .1, fill = primary) +
  geom_ribbon(aes(ymin = `...10`, ymax = `...26`), alpha = .1, fill = primary) +
  geom_ribbon(aes(ymin = `...11`, ymax = `...25`), alpha = .1, fill = primary) +
  geom_ribbon(aes(ymin = `...12`, ymax = `...24`), alpha = .2, fill = primary) +
  geom_ribbon(aes(ymin = `...13`, ymax = `...23`), alpha = .2, fill = primary) +
  geom_ribbon(aes(ymin = `...14`, ymax = `...22`), alpha = .2, fill = primary) +
  geom_ribbon(aes(ymin = `...15`, ymax = `...21`), alpha = .2, fill = primary) +
  geom_ribbon(aes(ymin = `...16`, ymax = `...20`), alpha = .2, fill = primary) +
  geom_ribbon(aes(ymin = `...17`, ymax = `...19`), alpha = .2, fill = primary) +
  geom_line(aes(col = Deaths, alpha = ifelse(time_value > t0_date, 1, .5)), key_glyph = "timeseries") + 
  geom_vline(xintercept = t0_date) +    geom_vline(xintercept = t0_date + h, lty = 2) +
  labs(x = "", y = "Deaths per 100k people") +
  theme(legend.title = element_blank()) + 
...
```{r q-reg-more-quantiles-sort}
#| fig-width: 7
pred_interval_rq_23_sort <- matrix(NA, nrow = n, ncol = 23)
for (t in t0:(n-h)) {
  # Fit quantile regression
  rq_trailing = rq(deaths ~ lagged_deaths + lagged_cases, 
                   tau = c(0.01, 0.025, 1:19 / 20, 0.975, 0.99),
                   data = ca, subset = (1:n) <= t & (1:n) > (t-w)) 
    pred_interval_rq_23_sort[t+h, ] = sort(pmax(0, predict(rq_trailing, 
                                                 newdata = data.frame(ca[t+h, ]))))
}

cbind(ca, pred_interval_rq_23_sort) |>
  mutate(observed = deaths, 
         `predicted (sorted)` = `...18`) |>
  pivot_longer(cols = c(`predicted (sorted)`, observed), 
               names_to = 'Deaths') |>
  ggplot(aes(time_value, value)) +
  geom_ribbon(aes(ymin = `...7`, ymax = `...29`), alpha = .1, fill = primary) +
  geom_ribbon(aes(ymin = `...8`, ymax = `...28`), alpha = .1, fill = primary) +
...
pred_aware |>
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = .3, fill = primary) +
  geom_line(aes(y = median), col = primary) +
  geom_vline(xintercept = t0_date) +    
  geom_vline(xintercept = t0_date + h, lty = 2) +
  labs(x = "", y = "Deaths per 100k people") +
  theme(legend.title = element_blank()) +
...
rq_pred_trailing |>
  ...
  geom_line(aes(y = deaths), col = 'gray50') + 
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = .3, fill = tertiary) +
  geom_line(aes(y = median), col = tertiary) +
  geom_vline(xintercept = t0_date) +    
  geom_vline(xintercept = t0_date + h, lty = 2) +
  labs(x = "", y = "Deaths per 100k people") +
  theme(legend.title = element_blank()) +
...
```{r qr-errors-unaware}
getErrors(rq_pred_trailing$deaths[!is.na(rq_pred_trailing$median)], 
          rq_pred_trailing$median[!is.na(rq_pred_trailing$median)], "version-unaware")
```
