...
other-authors: "Alice Cima, Rachel Lobay, and Logan C. Brooks"
...
ggplot(ca |> 
  ...
  scale_color_manual(values = palette()[c(2,4)]) +
  scale_y_continuous(
    name = "Covid-19 cases per 100k people", 
    limits = range1,
    sec.axis = sec_axis(
      trans = trans12, 
      name = "Deaths per 100k people")) +
  labs(title = "Covid-19 cases and deaths in California", x = "") +
  theme(legend.position = "bottom", legend.title = element_blank()) 
```
...
for (t in t0:(n-h)) {
  ...
  pred_all_past[t+h] = max(0, predict(arx_all_past, newdata = data.frame(ca[t+h, ])))
  pred_trailing[t+h] = max(0, predict(arx_trailing, newdata = data.frame(ca[t+h, ])))
}
```
...
ca |> 
  ...
  geom_line(aes(alpha = ifelse(time_value > as.Date("2021-09-13")-2*h-w &
                                 time_value < as.Date("2021-09-13") &
                                 Deaths == "observed", 1, .5), col = Deaths)) + 
  #geom_vline(xintercept = t0_date) +    
  #geom_vline(xintercept = t0_date + h, lty = 2) +
  geom_point(aes(x = as.Date("2021-09-13"), y = 0), size = 1, shape = 3, col = 'darkblue') + 
  geom_vline(xintercept = as.Date("2021-09-13")-2*h-w, lty = 3, col = 'darkred') +  
  geom_vline(xintercept = as.Date("2021-09-13")-h, lty = 3, col = 'darkred') +  
  labs(x = "", y = "Deaths per 100k people") +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  scale_alpha_identity()
```

```{r check-issue-trailing, eval=FALSE}
...
trailing
predict(all_past, newdata = data.frame(ca[t_issue+h, ]))
predict(trailing, newdata = data.frame(ca[t_issue+h, ]))
...
ca |> 
  mutate(observed = deaths, 
         `predicted, ARX(2)` = pred_arx2, 
         `predicted, ARX(3)` = pred_arx3) |>
  pivot_longer(cols = c(observed, `predicted, ARX(2)`, `predicted, ARX(3)`), 
               names_to = 'Deaths') |>
  ggplot(aes(time_value, value, col = Deaths)) +
  geom_line(aes(alpha = ifelse(time_value > t0_date, 1, .5))) + 
  geom_vline(xintercept = t0_date) +    
...
```{r arx-2-and-3-cv-error}
errors_arx <- rbind(getErrors(test_h$deaths, pred_arx2[!is.na(pred_arx2)],
                              "ARX(2), CV + trailing"),
                    getErrors(test_h$deaths, pred_arx3[!is.na(pred_arx3)],
                              "ARX(3), CV + trailing"))
errors_arx
```

```{r arx-multiple-lags, eval=FALSE}
...
rbind(getErrors(y_train, pred_train_ridge, "ARX(3) + ridge, training"),
      getErrors(y_test, pred_test_ridge, "ARX(3) + ridge, split-sample"),
      getErrors(y_train, pred_train_lasso, "ARX(3) + lasso, training"),
      getErrors(y_test, pred_test_lasso, "ARX(3) + lasso, split-sample"))
```

## Time-series CV for ARX + ridge/lasso (trailing)

```{r regularization-cv-worst, eval=FALSE}
h <- 28  # number of days ahead 
w <- 120 # window length
...
for (t in t0:(n-h)) {
  ...
  # Predict
  yhat_ridge[t+h] = max(0, predict(ridge_trail, newx = as.matrix(X[(t+h), ])))
  yhat_lasso[t+h] = max(0, predict(lasso_trail, newx = as.matrix(X[(t+h), ])))
}
```

```{r regularization-cv-ok, eval=FALSE}
h <- 28  # number of days ahead 
w <- 120 # window length
...
for (t in t0:(n-h)) {
  ...
  ridge_cv = cv.glmnet(as.matrix(X[cv_inds, ]), y[cv_inds], alpha = 0)
  lasso_cv = cv.glmnet(as.matrix(X[cv_inds, ]), y[cv_inds], alpha = 1)
  #if (t == t0 + h){plot(ridge_cv)}
  #if (t == t0 + h){plot(lasso_cv)}
  best_lambda_ridge = ridge_cv$lambda.1se
  best_lambda_lasso = lasso_cv$lambda.1se
  # Indices of data within window
  inds = t-w < 1:n & 1:n <= t
...
```{r plot-regularization-cv, eval=FALSE}
#| fig-align: left
ca |> 
  mutate(observed = deaths, 
         `predicted (ridge)` = yhat_ridge,
         `predicted (lasso)` = yhat_lasso) |>
  pivot_longer(cols = c(observed, `predicted (ridge)`, 
                        `predicted (lasso)`), names_to = 'Deaths') |>
  ggplot(aes(time_value, value, col = Deaths)) +
  geom_line(aes(alpha = ifelse(time_value > t0_date, 1, .5))) + 
...
rbind(getErrors(y_test, pred_arx3[!is.na(pred_arx3)], "ARX(3), CV + trailing"),
      getErrors(y_test, yhat_ridge[!is.na(yhat_ridge)], "ARX(3) + ridge, CV + trailing"),
      getErrors(y_test, yhat_lasso[!is.na(yhat_lasso)], "ARX(3) + lasso, CV + trailing"))
```
