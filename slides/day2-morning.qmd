...
ca |> 
  ...
  mutate(Deaths = factor(Deaths, 
                         levels = c("observed", "ARX(1)", "lm on lagged cases"))) |>
  ggplot(aes(time_value, value, col = Deaths)) +
  geom_line(aes(alpha = ifelse(time_value > t0_date, 1, .5))) + 
  geom_vline(xintercept = t0_date) +    
  geom_vline(xintercept = t0_date + h, lty = 2) +
...
```{r arx-cv-error}
rbind(getErrors(test_h$deaths, pred_arx1[!is.na(pred_arx1)],
                "ARX(1)"),
      getErrors(test_h$deaths, pred_trailing[!is.na(pred_trailing)],
                "lm on lagged cases"))
```
...
```{r regularization-train-test-errors, eval=FALSE}
