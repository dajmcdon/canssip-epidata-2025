...
* [**Revision**]{.primary} occurs when data is updated or corrected after initial publication, due to late reporting or new information.
...
1. [**Sources that don't revise**]{.primary} (provisional and final are the same) - Facebook or Google symptoms

1. [**Predictable revisions**]{.secondary} - Claims data (CHNG) and public health reports aligned by test, hospitalization, or death date

1. [**Revisions that are large and erratic to predict**]{.tertiary} - COVID cases and deaths, especially when aligned by report date 
...
* This underscores [**cross-checking data with external sources**]{.primary} not part of the same reporting systems.
...
1. **Be the one-stop shop for aggregated epi-surveillance time-series ("epi-signals")**
    + Includes signals from other sources, especially if they don't keep data revisions - E.g. CDC's own NSSP, NWSS
...
[**Epidata**]{.primary} is a repository of aggregated epi-surveillance time series. We aim to make everything [free and open-source]{.primary}.

<!-- To the full extent we can, we make everything [free and open-source]{.primary}. -->

* To date, it has accumulated over 5 billion records.

<!-- (each record is the value of a signal, at a particular date, and a particular location). -->
...
<!-- MSA, HRR, state, HHS region, nation -->
<!-- 1st point: Concepts of "reporting date" and "as of". -->
...
* The [main endpoint](https://cmu-delphi.github.io/delphi-epidata/api/covidcast.html) (covidcast) providing daily updates about COVID-19 and influenza in the U.S.
...
* Anonymous API access is subject to the following restrictions:
  1) public datasets only; 2) rate limit: 60 requests per hour; 3) only two parameters may have multiple selections
...
* **Function Overview**: `covidcast_epidata()` offers detailed insights from the COVIDcast endpoint.

<!-- detailed insights into data sources from the COVIDcast endpoint. -->

* **Source List**: Each data source is listed in `covid_sources$sources`, with tibbles describing the included signals.
...
## Returned data - COVIDcast main endpoint 
Crucially—and unlike most other COVID-19 data sources—our API reports two additional fields with each record:
...
* Meaning that it tracks the complete revision history of the signal.
...
there are private methods (starting with `pvt_`) that require private access keys.
To run these locally, store the keys in your `.Reviron` file, or set them as environmental variables.
...
* **Ex:** The last issue is from July 2020 and so the data is finalized as of that date (not the last requested). 
...
* Avoid queries with too-late minimum issue or too-early maximum issue, as it could lead to misleading results. You're safest bet to capture all issues is on the next slide...
...
* [**Delphi Epidata:**]{.primary} A platform for real-time epidemic data, providing aggregated signals for disease tracking and forecasting from diverse sources like [**health records**]{.secondary}, [**mobility patterns**]{.tertiary}, and [**more**]{.fourth-colour}.
...
* [**Epidata API:**]{.primary} Open-access API delivering up-to-date, granular epidemiological data + historical versions.

<!-- Open-access API delivering up-to-date, granular epidemiological data + makes all historical versions available. -->

* [**Epidatr:**]{.primary} Enables you to access Delphi's epidemiological data through R, offering powerful API functions.

<!-- and interactive tools for discovering and analyzing health signals. -->

* [**Versioned Data and Latency:**]{.primary} A standout feature of this API is its inclusion of two critical fields...

    1. `as_of`:  One version of the data, and referring to the specific date when the data was last updated 
    
    2. `issues`: Multiple versions of the data, each corresponding to different `as_of` dates.
