...
* Many epidemic aggregates are subject to [reporting delays]{.primary}
...
* The event time is indicated by `time_value`
...
4. **Provide access to otherwise private data where possible**
    + With privacy protocols in place
...
* A [full-featured R client](https://cmu-delphi.github.io/delphi-epidata/api/client_libraries.html) for quick access to all data

* A [legacy Python client](https://cmu-delphi.github.io/delphi-epidata/api/client_libraries.html#python), full-featured Python client in development
...
* ATTN: what do the pairs in { } vs ( ) mean? Standardize.
...
: %Covid by age group, by {county x day}
...
HHS Hosp/ICU beds
: Covid, flu, by age-group, by {state x day}, {facility x week}
...
* Displays [a selection](https://delphi.cmu.edu/covidcast/) of signals for COVID-19 & flu


* Browse by location or indicator

* Choose signals to export
...
<iframe width="800" height="800" src="https://delphi.cmu.edu/covidcast/indicator/?date=20240426&region=NC"></iframe>
...
Easily find data sources and signals, no programming required

[https://delphi.cmu.edu/signals/](https://delphi.cmu.edu/signals/)
...
County `geo_values` are [FIPS codes](https://en.wikipedia.org/wiki/List_of_United_States_FIPS_codes_by_county) 
...
To obtain the signal values for all available counties, replace `geo_values = "06059"` with `geo_values = "*"`.
:::
...
[**Versioning is important for forecasting**]{.primary}:
...
* Can matter if the latency or reporting lag is unknown
...
* At some point, nothing changes
* It is [finalized]{.primary}
