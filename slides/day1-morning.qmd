...
```{r}
#| echo = FALSE 
head(epidata) %>% select(geo_value, signal, source, geo_type, time_value, issue, lag, value)
```
...
```{r}
#| echo = FALSE 
head(fb_county_data) %>% select(geo_value, signal, source, geo_type, time_value, issue, lag, value)
```
</div>
...
1. **Predictable revisions** - Ex. Claims data (CHNG) and public health reports aligned by test, hosp., or death date

1. **Revisions that are large and erratic to predict** - Ex. COVID cases and deaths, especially when aligned by report date (which can be highly variable & less predictable compared to test data). 
...
* **Versioned Data and Latency:** Panel data captures time-series trends, which are often subject to revision.  A standout feature of this API is its inclusion of two critical fields...
    ...
    2. `issues`: Multiple versions of the data, each corresponding to different `as_of` dates, capturing revisions and updates over time.
