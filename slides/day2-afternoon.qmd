...
```{r error-arx}
test_h <- ca %>% filter(time_value %in% pred_arx$target_date)
...
```{r load-hhs-data}
source(here::here("_code/hospitalizations.R"))
```
...
pred_arx_hosp |>
  ggplot(aes(target_date, .pred)) +
  geom_line(data = ca_extend, aes(x = time_value, y = deaths), inherit.aes = FALSE, col = base) + 
  geom_line(col = primary) +
  geom_ribbon(aes(ymin = `0.1`, ymax = `0.9`), alpha = .3, fill = primary) +
  geom_vline(xintercept = t0_date) +    
  geom_vline(xintercept = t0_date + h, lty = 2) +
...
observed_pred_arx_hosp <- ca_extend |> filter(time_value %in% pred_arx_hosp$target_date)
getErrors(observed_pred_arx_hosp$deaths, pred_arx_hosp$.pred, "")
```
...
observed_pred_arx_more_lags <- ca_extend |> filter(time_value %in% pred_arx_more_lags$target_date)
getErrors(observed_pred_arx_more_lags$deaths, pred_arx_more_lags$.pred, "")
```
...
observed_pred_arx_rf <- ca_extend |> filter(time_value %in% pred_arx_rf$target_date)
getErrors(observed_pred_arx_rf$deaths, pred_arx_rf$.pred, "")
```
...
observed_pred_arx_grf <- ca_extend |> filter(time_value %in% pred_arx_grf$target_date)
getErrors(observed_pred_arx_grf$deaths, pred_arx_grf$.pred, "")
```
...
observed_ca_geo_pool <- ca_extend |> filter(time_value %in% pred_ca_geo_pool$target_date)
getErrors(observed_ca_geo_pool$deaths, pred_ca_geo_pool$.pred, "")
```
...
forecasts |>
  ...
  geom_point(aes(y = `0.5`, color = factor(time_value))) +
