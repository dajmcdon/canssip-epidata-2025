...
```{r error-arx-with-dv}
observed_pred_arx_hosp <- ca |> filter(time_value %in% pred_arx_hosp$target_date)
...
observed_pred_arx_more_lags <- ca |> filter(time_value %in% pred_arx_more_lags$target_date)
...
observed_pred_arx_rf <- ca |> filter(time_value %in% pred_arx_rf$target_date)
...
observed_pred_arx_grf <- ca |> filter(time_value %in% pred_arx_grf$target_date)
...
observed_ca_geo_pool <- ca |> filter(time_value %in% pred_ca_geo_pool$target_date)
...
pred_arx_geo_pool |>
  filter(geo_value %in% c("ca", "ma", "ny", "tx")) |>
  ggplot(aes(target_date, .pred)) +
  geom_point(data = usa_archive$DT |> 
              filter(geo_value %in% c("ca", "ma", "ny", "tx")), 
            aes(x = time_value, y = deaths, color = factor(version)),
    inherit.aes = FALSE, na.rm = TRUE, alpha = .5, size = 1) +
  geom_line(col = base) +
  geom_ribbon(aes(ymin = `0.1`, ymax = `0.9`), alpha = .3, fill = base) +
  geom_vline(xintercept = t0_date) +    
  geom_vline(xintercept = t0_date + 28, lty = 2) +
  facet_wrap(vars(geo_value), scales = 'free_y') +
  labs(x = "", y = "Deaths per 100k people") +
