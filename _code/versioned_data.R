  ...
  covid_archive <- covid_archive |>
    epix_slide(
      ...
      function(x, gk, rtv) {
        x |>
          group_by(geo_value) |>
          epi_slide_mean(case_rate, .window_size = 7L, .new_col_name = "case_rate_7d_av") |>
          epi_slide_mean(death_rate, .window_size = 7L, .new_col_name = "death_rate_7d_av") |>
          ungroup()
      }
    ) |>
    rename(
    ...
    epix_slide(
      ...
      function(x, gk, rtv) {
        x |>
          group_by(geo_value) |>
          epi_slide_mean(death_rate, .window_size = 7L, .new_col_name = "death_rate_7d_av") |>
          epi_slide_mean(dv, .window_size = 7L, .new_col_name = "dv_7d_av") |>
          ungroup()
          )
      }
    ) |>
