...
since_date <- strftime("2022-05-15", "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
...
can_prov_cases_tbl <- purrr::map2(commit_pages$data_url, commit_pages$date, function(url, date) {
  ...
  result <- raw %>%
    ...
    filter(!is.na(province), time_value > "2020-01-01") |>
...
write_rds(arch$DT, file = here::here("_data", "can_cases_before_2022_archive.rds"))
